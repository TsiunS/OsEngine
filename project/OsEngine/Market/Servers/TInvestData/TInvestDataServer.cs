...
using OsEngine.Market.Servers.TInvestData.Entity;
using RestSharp;
using System;
...
{
    public class TInvestDataServer : AServer
    {
        public TInvestDataServer()
        {
            TInvestDataServerRealization realization = new TInvestDataServerRealization();
            ServerRealization = realization;
            NeedToHideParameters = true;
        }
    }

    public class TInvestDataServerRealization : IServerRealization
    {
        #region 1 Constructor, Status, Connection

        public TInvestDataServerRealization()
        {
        ...
        private string _tradesDirectory = @"Data\TInvestDataStorage\Trades\";

        private string _candlesDirectory = @"Data\TInvestDataStorage\Candles\";

        private string _tempDirectory = @"Data\Temp\TInvestDataTempFiles\";
        ...
        private void GetSpotSecurities()
        {
            ...
            if (response != null)
            {
                ...
                var enumerator = root.Instruments.GetEnumerator();
        ...
        public List<Candle> GetCandleDataToSecurity(Security security, TimeFrameBuilder timeFrameBuilder, DateTime startTime, DateTime endTime, DateTime actualTime)
        {
            ...
            DateTime startTimeArchive = new DateTime(2018, 01, 23);

            if (startTime < startTimeArchive && endTime < startTimeArchive)
            {
                SendLogMessage("The data starts from 2018", LogMessageType.System);
                return null;
            }
            else if (startTime < startTimeArchive && endTime >= startTimeArchive)
            {
                SendLogMessage("The data starts from 2018", LogMessageType.System);
                startTime = startTimeArchive;
            }
            ...
            try
            {
              ...
              //  string[] dirBySecurity = Directory.GetDirectories(_candlesDirectory, $"*{secKey}*");

                for (int i = 0; i < urls.Count; i++)
                {
                    // поиск папок по бумаге в локальном хранилище //@"Data\TInvestDataStorage\Candles\SBER_TQBR_2018"
                   ...
                   if( Directory.Exists(_candlesDirectory + dirName)) // есть папка со свечками за этот год
                    {
                        ...
                        if (dailyFiles.Length > 0)
                        {
                            for (int j = 0; j < dailyFiles.Length; j++)
                            {
                                ...
                                List<Candle> dailyCandles = GetCandlesFromCsv(dailyFiles[j]);   // парсим файлы из папки 
                    ...
                    else
                    {
                        ...
                        if (zipArchivePath != null)
                        {
                            ...
                            if (dailyFiles.Length > 0)
                            {
                                for (int j = 0; j < dailyFiles.Length; j++)
                                {
                                    ...
                                    List<Candle> dailyCandles = GetCandlesFromCsv(dailyFiles[j]);
                        ...
                        {
                            SendLogMessage($"Candles data downloading error: archive path was null", LogMessageType.Error);
                            continue;
                        }
                    }
        ...
        private List<Candle> GetCandlesFromCsv(string csvFilePath)
        {
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                newCandle.Open = candleParts[2].ToDecimal();
                newCandle.Close = candleParts[3].ToDecimal();
                newCandle.High = candleParts[4].ToDecimal();
                newCandle.Low = candleParts[5].ToDecimal();
        ...
        public List<Trade> GetTickDataToSecurity(Security security, DateTime startTime, DateTime endTime, DateTime actualTime)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < dates.Count; i++)
                {
                    ...
                    int index = Array.IndexOf(archives, _tradesDirectory + dayStr + ".csv");

                    if (index != -1)
                    {
                         List<Trade> dailyTrades = csvFastParser.GetTradesFromCsv(archives[index], security);
                    ...
                    // архив есть, качаем
                    string gzipArchivePath = DownloadGZipArchive($"/history-trades/{dayStr}", dayStr, security.Name);  //https://invest-public-api.tinkoff.ru/history-trades/2026-03-21
        ...
        //    if (!File.Exists(dayArchivePath))
        //    {
        //        return trades;
        //    }

        //    // TRADE_TS, TICKER_CC, DIRECTION,PRICE, QUANTITY,TRADE_SOURCE,INSTRUMENT_UID
        //    // 2018-12-29T08:19:39.405017Z,GAZP_TQBR,BUY,153.29,10,EXCHANGE,962e2a95-02a9-4171-abd7-aa198dbe643a
        ...
        //        if (string.IsNullOrWhiteSpace(line) || line.StartsWith("TRADE_TS"))
        //        {
        //            continue;
        //        }

        //        string[] tradeParts = line.Split(',', StringSplitOptions.None);

        //        if (tradeParts.Length < 7)
        //        {
        //            continue;
        //        }

        //        if (!tradeParts[1].Equals(security.Name + "_" + security.NameClass))
        //            continue;
        ...
        //        DateTime tradeTime = DateTime.ParseExact(tradeParts[0], "yyyy-MM-dd'T'HH:mm:ss.ffffff'Z'", CultureInfo.InvariantCulture, DateTimeStyles.AssumeUniversal | DateTimeStyles.AdjustToUniversal);

        //        trade.Time = tradeTime.AddHours(3);
        //        trade.Side = tradeParts[2] == "BUY"  ? Side.Buy : Side.Sell;
        //        trade.Price = tradeParts[3].ToDecimal();
        //        trade.Volume = Math.Abs(tradeParts[4].ToDecimal());
        //        trade.Id = tradeParts[6];
        ...
        private string GetSCVFileFromArchive(string tempArchivePath, string nameFile)
        {
            string csvFilePath = _tradesDirectory + nameFile + ".csv";

            ExtractGZip(tempArchivePath, csvFilePath);
        ...
        private string DownloadDailyArchive(string instrumentId, DateTime day)
        {
            string requestPath = $"/history-trades/{day:yyyy-MM-dd}?instrumentId={instrumentId}";
            string tempGzipPath = Path.Combine(_tradesDirectory, $"{instrumentId}_{day:yyyyMMdd}.csv.gz");
            string csvPath = Path.Combine(_tradesDirectory, $"{instrumentId}_{day:yyyyMMdd}.csv");
            ...
            {
                SendLogMessage($"Error loading archive {instrumentId} {day:yyyy-MM-dd}. {ex.Message}", LogMessageType.Error);
                return null;
            }
        }
        ...
        private List<Trade> ParseCsvFileToTrades(string csvFilePath, string secName)
        {
            ...
            // TRADE_TS,TICKER_CC,DIRECTION,PRICE,QUANTITY,TRADE_SOURCE,INSTRUMENT_UID
            // 2026-03-21T00:00:01.407196Z,VTBR_TQBR,BUY,86.09,37,DEALER,8e2b0325-0292-4654-8a18-4f63ed3b0e09
